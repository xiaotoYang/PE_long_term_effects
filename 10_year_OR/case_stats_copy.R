...
race = read.csv("/nfs/turbo/umms-lgarmire2/haomingz-copy/long_term_effect_PE_3/case/PatientRace.csv")
social = read.csv("/nfs/turbo/umms-lgarmire2/haomingz-copy/long_term_effect_PE_3/case/ClaritySocialHistory.csv")


load("/nfs/turbo/umms-lgarmire2/Xiaotong/case_diag_date.RData")
load("/nfs/turbo/umms-lgarmire2/Xiaotong/case_como_before/case_como_200dbefore.RData")
...
# Two sets of data: *_elix - converted to ElixComo; w/o _elix - ICD codes
for (i in 1:10) {
...
for (i in 1:10) {
  ...
  case_diag_all_list[[i]] = diag_all_i
  case_diag_all_elix_list[[i]] = diag_all_elix_i
}

save(case_diag_all_list, file = "/nfs/turbo/umms-lgarmire2/Xiaotong/10yr_odds_ratio/case_diag_all_list.RData")
save(case_diag_all_elix_list, file = "/nfs/turbo/umms-lgarmire2/Xiaotong/10yr_odds_ratio/case_diag_all_elix_list.RData")

case_stats_list = list()
case_stats_elix_list = list()

# Basic statistics
for (i in 1:10) {
  case_stats_i = diag_hist_list[[i]] %>%
    group_by(TermCodeMapped) %>%
    summarise(count = n()) %>%
    mutate(Rate = count / pop_case_list[[i]])
  case_stats_list[[i]] = case_stats_i
...
save(case_stats_list, file = "/nfs/turbo/umms-lgarmire2/Xiaotong/10yr_odds_ratio/case_stats_list.RData")
save(case_stats_elix_list, file = "/nfs/turbo/umms-lgarmire2/Xiaotong/10yr_odds_ratio/case_stats_elix_list.RData")
